...
void Bird::update(bool i_move, std::vector<Pipes> i_pipes, sf::Window* window, sf::Event* ev)
{
	...
	if (0 <= vertical_speed && y >= BIRD_SIZE && Pressed && !dead)
	{
		Pressed = false;
		vertical_speed = -4.0f;
	}

	if (ev->type == sf::Event::KeyReleased && !Pressed)
		one = true;

	else if (1 == sf::Keyboard::isKeyPressed(sf::Keyboard::S))
			vertical_speed = 0.5f;

	if (y < GROUND_Y - BIRD_SIZE && y + vertical_speed < GROUND_Y - BIRD_SIZE)
		y += vertical_speed;

	else if (y + vertical_speed > GROUND_Y - BIRD_SIZE)
	{
